      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {cards.map((card, index) => (
            <div 
              key={index}
              className="group relative rounded-2xl p-10 border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300"
            >
              {/* Right Half Circle Decoration - increases on hover */}
              <div className={`absolute -right-16 top-1/2 -translate-y-1/2 w-40 h-42 ${card.bgColor} rounded-full opacity-60 group-hover:scale-140 group-hover:opacity-80 transition-all duration-500 ease-in-out`}></div>
